...
| 3 | `/ask Name` | Auto-pulls question from slide config, plays question audio |
...
> **`/ask` command formats:**
> - `/ask Maria` — auto-pulls the question for the current slide (recommended)
> - `/ask Maria: Custom question?` — override with your own question
>
> The **answer is always live** (generated by Claude + spoken by ElevenLabs TTS).
> If you use a custom question, the pre-recorded question audio won't match — the
> system will attempt to play the slide's audio anyway as a fallback.
>
> **Commands are blocked while audio is playing.** Wait for audio to finish,
> or use `/skip` to interrupt and proceed immediately.
...
/ask Maria                                     → Auto: "What's one task that feels repetitive?"
(type their answer summary)
/next                                          → Slide 3: What AI Is
/ask Jake                                      → Auto: "Ever been surprised by how wrong AI was?"
(type their answer summary)
/next                                          → Slide 4: ChatGPT
/ask Sarah                                     → Auto: "Do you use ChatGPT? Go-to use case?"
(type their answer summary)
/next                                          → Slide 5: Ecosystem
/ask Tom                                       → Auto: "Microsoft, Google, or a mix?"
(type their answer summary)
/next                                          → Slide 6: Advanced Workflows
/ask Lisa                                      → Auto: "Automated any part of your workflow?"
(type their answer summary)
/next                                          → Slide 7: Entertainment
/ask Ben                                       → Auto: "Tried AI for a creative project?"
(type their answer summary)
...
/ask Name           AI asks Name the question for the current slide (auto)
/ask Name: Q        AI asks Name a custom question (override)
...
/skip               Skip current action / interrupt audio
